...
void IA32::Context::load() const volatile
{
    ...
    ASM("mov $0x23, %ax");               // Seletor do segmento de dados
    ASM("mov %ax, %ds");
    ASM("mov %ax, %es");
    ASM("mov %ax, %fs");
    ASM("mov %ax, %gs");

    ASM("popl %esi");                    // Obtém o valor da pilha atual
    ASM("pushl $0x23");                  // Salva o novo Stack Segment (SS)
    ASM("pushl %0" : : "r"(System::info()->lm.app_heap)); // Salva o endereço da pilha
    ASM("pushf");                        // Salva o EFLAGS

    ASM("pushl $0x1B");                  // Salva o CS
    ASM("push %esi");                    // Endereço da pilha
...
void IA32::switch_context(Context * volatile * o, Context * volatile n)
{
    ...
    //TODO: alterar o tss com informações da próxima thread (CPU::switch_tss)
